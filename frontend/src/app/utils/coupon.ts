export const verifyCouponApplicable = (couponData, total = 0) => {
  if (
    ...
    couponData == null ||
    total == 0 ||
    couponData.min_purchase > total
  ) {
    return {
      status: false,
      message:
        'coupon can not be applied on 0 or invalid coupon or minimum purchase must be greater or equal to ' +
  ...
  var couponDiscount =
    couponData.discount_type == 'amount'
      ? couponData.discount
      : (total / 100) * couponData.discount > couponData.max_discount
      ? couponData.max_discount
      : (total / 100) * couponData.discount;
  var discountCondition = couponDiscount > total;

  if (discountCondition) {
    return {
      status: false,
      message: 'coupon can not be applied, your cart subtotal is low',
    };
  }
...
export const ApplyCouponsService = {};
